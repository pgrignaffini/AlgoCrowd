...
def test_funding_no_opt_in():
    ...
    with pytest.raises(Exception):
        ...
        sendFunds(client=client, appID=appID, funder=funder, platform=platform, fundAmount=fundAmount)
...
def test_goal_met():
    ...
    assert creatorBalance == creatorAlgosBefore + APP_STARTING_BALANCE + fundAmount - PLATFORM_FEE - 2 * 1_000
...
def test_refunds():
    ...
    assert funderAlgosBefore - 500 >= funderBalanceAfterOptIn >= funderAlgosBefore - 1000
    ...
    assert funderBalanceAfterOptIn - fundAmount - 2 * 1000 >= funderBalanceAfterSendingFunds >= funderBalanceAfterOptIn - fundAmount - 3 * 1000
    ...
    assert funderBalanceAfterSendingFunds + fundAmount - PLATFORM_FEE - 1000 >= funderBalanceAfterGettingRefund >= funderBalanceAfterSendingFunds + fundAmount - PLATFORM_FEE - 2 * 1000

    closeCrowdfunding(client, appID, creator)

    platformBalance = getBalances(client, platform.getAddress())[0]

    assert platformBalance == TEMP_ACCOUNT_BALANCE + PLATFORM_FEE
